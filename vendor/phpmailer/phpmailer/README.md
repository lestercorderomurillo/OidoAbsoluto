...
- SMTP AuthenticationMiddleware with LOGIN, PLAIN, CRAM-MD5, and XOAUTH2 mechanisms over SMTPS and SMTP+STARTTLS transports
...
Many PHP developers need to send email from their code. The only PHP function that supports this directly is [`mail()`](https://www.php.net/manual/en/function.mail.php). However, it does not provide any assistance for making use of popular features such as encryption, AuthenticationMiddleware, HTML messages, and attachments.
...
If you want to use the Gmail XOAUTH2 AuthenticationMiddleware class, you will also need to add a dependency on the `league/oauth2-client` package in your `composer.json`.
...
try {
    ...
    $mail->SMTPAuth   = true;                                   //Enable SMTP AuthenticationMiddleware
...
- CRAM-MD5 AuthenticationMiddleware support.
