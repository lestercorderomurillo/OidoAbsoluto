...
use Pipeline\PypeDOM\HTML\TagStrip;
use Pipeline\PypeDOM\HTML\Selection;
use Pipeline\PypeDOM\HTML\BodyFinder;
...
class PypeCompiler
{
    ...
    public static function setPypeDOM(PypeContextFactory $context_factory)
    {
    ...
    public static function renderString(string $this_html, array $this_context = [], int $depth = 0): string
    {
        ...
        try {
            ...
            while (($definition_selection = Pattern::selectStringByQuotes($output, "<", ">", $offset, 1))->isValid()) {
                ...
                if ($keyword != null) {
                    $component_object = self::elementStringToArray($definition_selection->getString());
                    $domtag = substr($component_object[0], strlen($keyword) + $closure);
                    $attributes = safe($component_object[1], []);


/*
                    $component_object = PypeCompiler::elementStringToArray($definition_selection->getString());
                    ...
                    switch (trim($keyword)) {
                        ...
                        case "this":
                            ...
                            if (isset($this_context["this:class"])) {
                                $value = Pattern::substituteTokens($this_context["this:class"], $this_context);
                                if ($value != null) {
                            ...
                            $template = $this->template_batch->getTemplate($this_context["this"]);
                        ...
                        case "if":
                            ...
                            if (!$closure) {
                                ...
                                if (strlen($value) > 0) {
                                    ...
                                    $mixed_context = Collection::mergeList(
                                        self::$context_factory->getViewContext(),
                                        self::$context_factory->getSessionContext(),
                                        $this_context
                                    );
                                                ...
                                                foreach ($mixed_context[$from_name] as $array => $object) {
                                                    ...
                                                    if ($skipped < $skip) {
                                                        ...
                                                        if ($taken < $take) {
                                                            ...
                                                            $mixed_context = Collection::mergeList($mixed_context, $local_context);
