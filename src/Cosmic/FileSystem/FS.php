...
/**
 * This class represents an abstraction for the FS of your OS. Developers can
 * load, write and read files and directories very easily using the path classes.
 */
class FS
{
    /**
     * Search all web-public files within a directory. Can be used with an specific extension.
     * The search will convert all paths to their public web variant.
     * If the extension is php, this will throw an exception.
     * 
     * @param FolderPath $folder The directory path to search.
     ...
     */
    public static function lookup(FolderPath $folder, string $extension = "*"): array
    {
        ...
        return FS::toWebPaths(FS::find($folder, $extension));
    }

    /**
     * Search all files within a directory(or directories). Can be used with an specific extension.
     * 
     * @param FolderPath[]|string[]|FolderPath|string $folders The directory path to search. Can be an array of folders.
     ...
     * 
     * @return FilePath[] A collection of files path.
     */
    public static function find($folders, $extensions = "*"): array
    {
        $folders = Collections::normalizeToList($folders);
        $extensions = Collections::normalizeToList($extensions);

        $paths = [];

        foreach ($folders as $folder) {

            $folder = FS::normalizeToFolderPath($folder);

            foreach ($extensions as $extension) {
                $searchPath = $folder . "*.$extension";

                foreach (FS::recursiveGlob($searchPath) as $file) {
                    $paths[] = new FilePath($file);
                }
            }
        }
     ...
     * 
     * @param FilePath|string $file The file path to import.
     ...
     */
    public static function import($file, bool $required = true, bool $once = false)
    {
        try {

            $file = FS::normalizeToFilePath($file);
            ...
            throw new IOException("Failed to import the requested file: $file <br>Internal Error: " . $th->getMessage());
        }
    }

    /**
     * Verify if the given path file exists.
     * 
     * @param FilePath|FolderPath $path The file or directory to check.
     ...
     * 
     * @param FilePath|string $file The file to read.
     * @return string|false The content of the file as a string, false on error.
     */
    public static function read($file): string
    {
        return file_get_contents(FS::normalizeToFilePath($file));
    }

    /**
     * Normalize the given string to a new file if  necessary.
     * 
     * @param FilePath|string $file The file to normalize.
     * @param FilePath The output file.
     */
    public static function normalizeToFilePath($file): FilePath
    {
        if(is_string($file)) {
    ...
    /**
     * Normalize the given string to a new folder if  necessary.
     * 
     * @param FolderPath|string $folder The file to normalize.
     * @param FolderPath The output file.
     */
    public static function normalizeToFolderPath($folder): FolderPath
    {
        if(is_string($folder)) {
            $folder = new FolderPath($folder);
        }
     ...
     * 
     * @param FilePath|string $path The file to write.
     ...
     * 
     */
    public static function write($file, string $value, int $mode = FILE_APPEND)
    {
        $file = FS::normalizeToFilePath($file);
    ...
    {
        $output = [];
        $paths = Collections::normalizeToList($paths);
