...
use Cosmic\DOM\Exceptions\NotFoundComponentException;
use Cosmic\Utilities\Collections;
use Cosmic\Utilities\HTML;

/**
 * This class represents the cosmic DOM tree.
 */
class Bindings
{
    /**
    ...
    public function getComponentClassName(string $tag): string
    {
        try {
            ...
            throw new NotFoundComponentException("DOM doesn't have the component \"$tag\" registered yet. \nFrom Internal: " . $exception->getMessage());
        }
    }
    ...
    {
        app()->primitive(Component::getPublishName($className), $className);
    }
    ...
    public function getSCSSFiles(): array
    {
        ...
        foreach ($all as $key => $value) {
            if (str_starts_with($key, "Component@")) {
                $value = $value->get();
                $files = Collections::merge($files, Component::getStyleFilesPath($value));
            }
        }
