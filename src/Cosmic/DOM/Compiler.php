...
use Cosmic\DOM\HTML\Beautifier;
...
class Compiler
{
    ...
    public function compileStylesheet()
    {
        if (configuration("framework.compileStylesheet")) {
            ...
            $outputFile = new FilePath(__CONTENT__ . "Output/Build.css");

            // Content loading
            $baseContent = FileSystem::read(new FilePath(__CONTENT__ . "page.scss")) . "\n\n";

            // Get prefixes files
            $packages = FileSystem::find(new FolderPath("src/Cosmic/Bundle/Packages/"), ["scss"]);

            // Get components files
            $files = Collections::merge($packages, app()->get(Bindings::class)->getSCSSFiles());
                ...
                if (!$isCloseTag) {
                    ...
                    if ($this->isGenericElement($elementString)) {
                        ...
                        if ($component->isInlineComponent()) {
                            ...
                            $passdownTokens = Collections::merge($tokens, $componentTokens);
                            $selection->setEndPosition($selection->getEndPosition());
                        } else {

                            $passdownTokens = Collections::merge($tokens, $componentTokens, ["body" => $component->body]);
        ...
        while (($selection = Pattern::select($html, "{", "}", $offset))->isValid()) {
            ...
            if (strlen($tokenRaw) > 0 && $tokenRaw[0] != "?" && $tokenRaw[0] != '"' && isset($tokens[$tokenRaw])) {
                ...
                if (is_array($tokens[$tokenRaw])) {
                    $compiledValue = Collections::encodeArraytoString($tokens[$tokenRaw]);
                } else {
    ...
    public static function extractRawData(string $input): array
    {
        ...
        if (isset($raw[1])) {
            ...
            $attributes = StrquotedExplode($raw[1]);

            foreach ($attributes as $attribute) {

                $attribute = StrmultiExplode(["='", "=\""], $attribute);
    ...
    public function compileMathExpression(string $html): string
    {
        ...
        while (($selection = Pattern::select($html, "[", "]", $offset))->isValid()) {
            ...
            if (!StrcontainsOne($expressionRaw, ["{", "}", '"'])) {
