...
class Compiler
{
    ...
    {
        $value = str_replace('{{', "<<<'HTML'", FileSystem::read($phpsFile));
    ...
    public function compileStylesheet()
    {
        if (configuration("framework.compileStylesheet")) {
            ...
            // Content loading
            $baseContent = "// AUTOGENERATED CSS FILE. DO NOT EDIT.\n";
            ...
            FileSystem::write($outputFile, $build->getCss(), 0);
        }
    }
     ...
     */
    public function compileTokens(string $html, array $tokens = []): string
    {
        $offset = 0;

        while (($selection = Pattern::select($html, "{", "}", $offset))->isValid()) {

            $tokenRaw = trim($selection->getString(true));

            if (isset($tokens[$tokenRaw])) {
     ...
     */
    public function compileTokensInArray(array $array, array $tokens = []): array
    {
        foreach ($array as $line) {
            $output[] = $this->compileTokens($line, $tokens);
        }
        return $output;
    }
