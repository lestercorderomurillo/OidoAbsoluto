...
class Component extends HTMLObject
{
    ...
    public function __construct(string $tag, array $attributes = [])
    {
        ...
        if (!Template::isRegistered($this->getDOMTag())) {
            (ResponseFactory::createServerResponse(500, "Trying to render the component \"$tag\" failed. <br> 
            Unregistered component. Check your view/components files."))->sendAndDiscard();
        }
    }
    ...
    public function render(): string
    {
        ...
        for ($depth = $this->getTransverseDepth(); $depth >= 0; $depth--) {
            $html_defaults = ArrayHelper::mergeNamedValues((new Template($this->getDOMTag(), $depth))->get("defaults", []), $html_defaults);
        }
        ...
        if ($function != NULL && $template->has("prototype")) {
            (ResponseFactory::createServerResponse(500, "Component \"" . $this->getDOMTag() . "\"
